...
t <- mclapply(unique(gene_chr$chr), function(CHR) {
  ...
  geneCluster <- do.call(rbind, lapply(seq(1:length(ap_result@clusters)), function(i) cbind(names(ap_result@clusters[[i]]), i))) %>%
    as.data.frame() %>%
    "colnames<-"(c("gene", "cluster"))
  geneBlackListed <- lapply(seq(1:length(ap_result@clusters)), function(i) {
    unlist(lapply(names(ap_result@clusters[[i]]), function(x) {
      if (length(which(mat.sub[x, names(ap_result@clusters[[i]])] > 0)) == 1) {
        x
      }
    }))
  }) %>% unlist()
...
geneCluster <- geneCluster %>%
  ...
  dplyr::select(c("chr", "start", "end", "gene", "iScore", "strand", "cluster")) %>%
  dplyr::arrange(chr, start, end)
geneCluster <- geneCluster %>% mutate(
  id = sprintf("%s_%s_%s", chr, start, end),
...
## @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@##
saveRDS(list(dfO = dfO, mat = mat, gene_chr = gene_chr, geneCluster = geneCluster), file = sprintf("%s.Rds", opt$name))
