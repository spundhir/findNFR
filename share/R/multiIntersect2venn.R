...
option_list <- list(
	...
	make_option(c("-t", "--type"), default="ellipses", help="type of plot; ellipses, ChowRuskey or ggvenn. (default: %default)"),
...
if(opt$type=="ggvenn") {
    p1 <- ggvenn(lst)
    ggsave(opt$outFile, p1)
} else if(length(names(lst)) <= 3 & opt$type=="ellipses") {
